...
class CifarResNet(nn.Module):
    ...
    def __init__(self, block, depth, channels=3, args=None):
        ...
        self.conv_1_3x3 = nn.Conv2d(channels, self.init_width, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn_1 = nn.BatchNorm2d(self.init_width)

        self.inplanes = self.init_width
        self.stage_1 = self._make_layer(block, self.init_width, layer_blocks, 1)
        self.stage_2 = self._make_layer(block, self.init_width*2, layer_blocks, 2)
        self.stage_3 = self._make_layer(block, self.init_width*4, layer_blocks, 2)
        self.avgpool = nn.AvgPool2d(8)
        self.out_dim = self.init_width*4 * block.expansion
...
def resnet20(args=None):
    ...
    model = CifarResNet(ResNetBasicblock, 20, args=args)
    return model


def resnet32(args=None):
    """Constructs a ResNet-32 model for CIFAR-10."""
    model = CifarResNet(ResNetBasicblock, 32, args=args)
    return model
